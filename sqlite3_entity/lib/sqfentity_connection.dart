...
class Sqlite3EntityConnectionMobile extends Sqlite3EntityConnectionBase {
  Sqlite3EntityConnectionMobile(Sqlite3EntityConnection? connection)
      : super(connection);
  @override
  Future<void> writeDatabase(ByteData data) async {
    ...
    try {
      final path = join(await getFinalDatabasePath(), connection!.databaseName);
  ...
  Future<Database> openDb() async {
    ...
    await lock.synchronized(() async {
      ...
      // uncomment line below if you want to use SQLite3
      _db = sqlite3.open(path); // SQLite3
      await createDb(_db!, connection!.dbVersion);
      // uncomment line below if you want to use sqflite
      // await openDatabase(path, version: connection!.dbVersion, onCreate: createDb); // SQFLite
    });
    //}
    return _db!;
  }
