...
import 'package:sqlite3_entity/sqfentity_connection.dart';
import 'package:sqlite3_entity/sqfentity_connection_base.dart';
//import 'package:sqlite3_entity/sqfentity_connection_ffi.dart';
...
class Sqlite3EntityProvider extends Sqlite3EntityModelBase {
  Sqlite3EntityProvider(Sqlite3EntityModelProvider dbModel,
      {String? tableName,
    ...
    _connection = connection ??
        Sqlite3EntityConnection(_dbModel!.databaseName!,
    ...
    _connectionBase = Sqlite3EntityConnectionMobile(_connection!);
    // } else {
    //   _connectionBase = Sqlite3EntityConnectionFfi(_connection!);
    // }
  }
  Sqlite3EntityProvider._internal();
  static final Sqlite3EntityProvider _sqfEntityProvider =
      Sqlite3EntityProvider._internal();
  static Sqlite3EntityProvider get = _sqfEntityProvider;
  String? _tableName = '';
  List<String>? _primaryKeyList;
  String? _whereStr;
  Sqlite3EntityModelProvider? _dbModel;
  Sqlite3EntityConnection? _connection;
  Sqlite3EntityConnectionBase? _connectionBase;
  ...
  Future<void> closeDatabase() async {
    if (_dbMap != null && _dbMap![_dbModel!.databaseName!] != null) {
       _dbMap![_dbModel!.databaseName!]!.dispose();
      _dbMap![_dbModel!.databaseName!] = null;
    }
  }
  ...
  Future<dynamic> getById(List<dynamic>? ids) async {
    ...
    final result = db.select(query, ids);
    return result;
  }
  ...
  Future<BoolResult> execSQL(String pSql, [List<dynamic>? arguments]) async {
    ...
    try {
      ...
      if (_dbMap![_dbModel!.databaseName!] != null) {
        ...
        throw Exception('SQL command could not executed due to the database is closed');
      }
    } catch (e) {
  ...
  Future<BoolCommitResult> execSQLList(List<String> pSql,
    ...
    for (String sql in pSql) {
      ...
      _dbMap![_dbModel!.databaseName!]!.execute(sql);
    }
    // if (closeBatch) {
    //   try {
    //     result
  ...
  Future<List<dynamic>> getForeignKeys(String tableName) async {
    final Database db = (await this.db)!;
    final result = db.select('PRAGMA foreign_key_list(`$tableName`)');
  ...
  Future<List<Map<String, Object?>>> execDataTable(String pSql,
    ...
    final Database db = (await this.db)!;
    final result = db.select(pSql, arguments);

    return result;
  }

  /// Run Select Command and return first col of first row
  Future<dynamic> execScalar(String pSql, [List<Object?> arguments =const []]) async {
    final Database db = (await this.db)!;
  ...
  Future<List> toList(QueryParams params) async {
    ...
    /*
    print('********** Sqlite3EntityProvider.toList(QueryParams=> columns:' +
  ...
  Future<int> sequence(Sqlite3EntitySequenceBase seq, bool increase,
...
abstract class Sqlite3EntityModelProvider extends Sqlite3EntityModelBase {
  Sqlite3EntityModelProvider({this.connection});
  Sqlite3EntityConnection? connection;
  ...
  Future<bool> initializeDB() async {
    ...
    if (dbSequences.isNotEmpty) {
      final tableSquence = await Sqlite3EntityProvider(this)
          .execDataTable('PRAGMA table_info(sqfentitysequences)');
      if (tableSquence.isEmpty) {
        await Sqlite3EntityProvider(this).execSQL(
            'Create table sqfentitysequences (id text UNIQUE, value integer)');
      }
      for (Sqlite3EntitySequenceBase sequence in dbSequences) {
        final sqRow = await Sqlite3EntityProvider(this).execDataTable(
    ...
    if (dbTables.isNotEmpty) {
      ...
      for (Sqlite3EntityTableBase table in dbTables) {
        switch (table.objectType) {
          case ObjectType.view:
            await Sqlite3EntityProvider(this).execSQLList([
...
${table.sqlStatement}'''
          ...
          case ObjectType.table:
            ...
            final tableFields = await Sqlite3EntityProvider(this)
            ...
            if (tableFields.isNotEmpty) {
              ...
              if (alterTableColsQuery.isNotEmpty) {
                ...
                final result = await Sqlite3EntityProvider(this)
            ...
            {
              final createTable = await Sqlite3EntityProvider(this)
                  .execSQL(table.createTableSQL);
              if (createTable.success) {
                ...
                if (alterTableIndexesQuery.isNotEmpty) {
                  await Sqlite3EntityProvider(this)
  ...
  Future<bool> initializeForeignKeys() async {
    ...
    for (final table in databaseTables!) {
      ...
      if (rfields.isNotEmpty) {
        final fKeys =
            await Sqlite3EntityProvider(this).getForeignKeys(table.tableName!);
    ...
    if (alterTableQuery.isNotEmpty) {
      ...
      await Sqlite3EntityProvider(this).execSQLList(alterTableQuery);
    }
    return true;
  }
  ...
  Future<BoolResult> execSQL(String sql, [List<dynamic>? arguments]) async {
    return Sqlite3EntityProvider(this).execSQL(sql, arguments);
  }

  /// Write database on existing db (path=your new database path, byte= your new database's ByteData)
  Future<void> writeDatabase(ByteData data) async {
    return Sqlite3EntityProvider(this).writeDatabase(data);
  }

  /// Run sql command List
  Future<BoolCommitResult> execSQLList(List<String> sql) async {
    return Sqlite3EntityProvider(this).execSQLList(sql);
  }
  ...
  Future<List<Map<String, Object?>>> execDataTable(String sql,
    ...
    return Sqlite3EntityProvider(this).execDataTable(sql, arguments);
  }

  /// Run Select Command and return first col of first row
  Future<dynamic> execScalar(String sql, [List<dynamic>? arguments]) async {
    return Sqlite3EntityProvider(this).execScalar(sql, arguments);
  }
  ...
  Future<bool> batchStart() async {
    return await Sqlite3EntityProvider(this).batchStart();
  }

  Future<void> closeDatabase() async {
    await Sqlite3EntityProvider(this).closeDatabase();
  }
  ...
  Future<List<dynamic>?> batchCommit(
      {bool? exclusive, bool? noResult, bool? continueOnError}) async {
    return Sqlite3EntityProvider(this).batchCommit(
        exclusive: exclusive,
        noResult: noResult,
        continueOnError: continueOnError);
  }
  ...
  /// Cancels all of the operations in this batch
  void batchRollback() async {
    return Sqlite3EntityProvider(this).batchRollback();
  }
}

/// Check the tables if initialized
bool checkForIsReadyDatabase(List<Sqlite3EntityTableBase> dbTables) {
...
List<String> checkTableIndexes(Sqlite3EntityTableBase table) {
  final alterTableQuery = <String>[];
  final List<String> addedIndexes = <String>[];

  for (Sqlite3EntityFieldType field in table.fields!) {
...
List<String> checkTableColumns(
    Sqlite3EntityTableBase table, List<TableField> existingDBfields) {
  final alterTableQuery = <String>[];
  ...
  for (var newField in table.fields!) {
    if (newField is Sqlite3EntityFieldVirtualBase) {
      continue;
    }
    ...
    if (eField.isNotEmpty) {
      ...
      if (newField is Sqlite3EntityFieldRelationshipBase) {
        alterTableQuery.add(
...
Future<Sqlite3EntityModelBase> convertDatabaseToModelBase(
    Sqlite3EntityModelProvider model,
    {String? bundledDatabasePath,
    String? databasePath}) async {
  final bundledDbModel = Sqlite3EntityProvider(model);
  ...
  final List<Sqlite3EntityTableBase> tables =
...
Future<List<Sqlite3EntityTableBase>> getObjects(
    List objectList,
    Sqlite3EntityProvider bundledDbModel,
    Sqlite3EntityModelProvider model) async {
  ...
  for (final table in objectList) {
    ...
    final existingDBfields = <Sqlite3EntityFieldType>[];
    if (tableFields.isNotEmpty) {
      ...
      /// convert table fields to Sqlite3EntityField
      for (int i = 0; i < tableFields.length; i++) {
        if (tableFields[i]['name'].toString() != primaryKeyName) {
          existingDBfields.add(Sqlite3EntityFieldBase(
              tableFields[i]['name'].toString(),
              parseDbType(tableFields[i]['type'].toString()),
              isPrimaryKeyField:
    ...
    tables.add(Sqlite3EntityTableBase()
      ..tableName = tableName
  ...
  for (var table in tables) {
    final relationFields = <Sqlite3EntityFieldRelationshipBase>[];
        ...
        for (final parentTable in tables) {
          if (parentTable.tableName!.toLowerCase() ==
            ...
            if (table.primaryKeyName!.toLowerCase() ==
              ...
              relationFields.add(Sqlite3EntityFieldRelationshipBase(
  ...
  for (var table in tables) {
    if (table.fields!.length == 2 &&
        table.fields!.whereType<Sqlite3EntityFieldRelationshipBase>().length ==
            2) {
      final ref = table.fields![0] as Sqlite3EntityFieldRelationshipBase;
      final referred = table.fields![1] as Sqlite3EntityFieldRelationshipBase;
      ref.table!.fields!.add(
          Sqlite3EntityFieldRelationshipBase(referred.table, ref.deleteRule)
