...
// Define the 'TableCategory' constant as Sqlite3EntityTable.
const Sqlite3EntityTable tableCategory = Sqlite3EntityTable(
    tableName: 'category',
    primaryKeyName: 'id',
...
// Define the 'TableProduct' constant as Sqlite3EntityTable.
const Sqlite3EntityTable tableProduct = Sqlite3EntityTable(
    tableName: 'product',
    primaryKeyName: 'id',
    primaryKeyType: PrimaryKeyType.integer_auto_incremental,
    useSoftDeleting: true,
...
const Sqlite3EntityTable tableTodo = Sqlite3EntityTable(
    tableName: 'todos',
    primaryKeyName: 'id',
    useSoftDeleting:
...
const Sqlite3EntitySequence seqIdentity = Sqlite3EntitySequence(
...
// STEP 3: Create your Database Model constant instanced from Sqlite3EntityModel
// Note: Sqlite3Entity provides support for the use of multiple databases. So you can create many Database Models and use them in the application.
@Sqlite3EntityBuilder(myDbModel)
const Sqlite3EntityModel myDbModel = Sqlite3EntityModel(
    modelName: null,
...
Future<void> printListDynamic(Sqlite3EntityProvider model, String pSql) async {
...
Future<String> createSqlite3EntityModelString([bool setClipboard = true]) async {
  ...
  final model = Sqlite3EntityModelConverter(myDbModel).toModelBase();
  final strModel = StringBuffer()
    ..writeln('part of \'model.dart\';')
    ..writeln(Sqlite3EntityConverter(model).createModelDatabase())
    ..writeln(Sqlite3EntityConverter(model).createEntites());
...
Future<void> samples10() async {
  print('EXAMPLE 10 Sqlite3Entity Sequence SAMPLES-----------');
...
void sampleModelConvert() {
  final seq = Sqlite3EntitySequenceBase()
    ..sequenceName = 'sample'
    ..init();
  ...
  final model = MyDb()
    ...
    ..sequences = [seq]
    ..databaseTables = [
      Sqlite3EntityTableBase()
        ..dbModel = 'myyymodelll'
        ..defaultJsonUrl = 'none'
        ..fields = [
          Sqlite3EntityFieldBase('name', DbType.text)..defaultValue = 'test',
          Sqlite3EntityFieldBase('identity', DbType.text, sequencedBy: seq)
            ..defaultValue = 'test'
        ]
        ..primaryKeyName = 'id'
        ..primaryKeyType = PrimaryKeyType.integer_auto_incremental
  ...
  final modelStr = Sqlite3EntityConverter(model).createEntites();
  print(modelStr);
}

class MyDb extends Sqlite3EntityModelBase {}
