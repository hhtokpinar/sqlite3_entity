...
Sqlite3Entity also generates add/edit forms with validations and special controls (DropDown List, DateTime pickers, Checkboxes.. etc) for your table.

Leave the job to SqfEntitiy for CRUD operations. Do easily and faster adding tables, adding columns, defining multiple tables, soft deleting, recovery, syncronize data from the web and more with the help of Sqlite3EntityTable class.
...
This project is a starting point for a Sqlite3Entity ORM for database application.
...
**STEP 1:**
For example, we have created 3 tables constant for category, product and todo that instanced from "Sqlite3EntityTable" as follows:
...
If this table (Product) is the child of a parent table (Category), you must declare the Sqlite3EntityFieldRelationship column into fields for Object Relational Mapping.
...
_And add a Sequence for samples_
...
**STEP 2**: Create your Database Model to be instanced from Sqlite3EntityModel
_Note:_ Sqlite3Entity provides support for the use of **multiple databases**.
...
_bundledDatabasePath_ is optional. When bundledDatabasePath is empty then EntityBase creats a new database when initializing the database
...
**bundledDatabasePath:** File path of your copied database
    ...
    class BundledDbModel extends Sqlite3EntityModelProvider {}
...
Run this function to convert the model to annotation
    ...
    final String modelConstString =
        Sqlite3EntityConverter(bundledDbModel).createConstDatabase();
...
_Note: save() method returns the primary id of the added record_
...
### EXAMPLE 10 Sqlite3Entity Sequence Samples
...
![Sqlite3Entity Sample Mobile Application for Flutter](https://raw.githubusercontent.com/hhtokpinar/sqfEntity/master/example/assets/img/Sqlite3Entity_Flutter_Mobile_Sample_Application.gif)
...
- Search Form with Sqlite3Entity search
...
### Running the main.dart should show the following result at DEBUG CONSOLE:
    ...
    flutter >>>>>>>>>>>>>>>>>>>>>>>>>>>> Sqlite3EntityTable of 'category' initialized successfuly
    flutter >>>>>>>>>>>>>>>>>>>>>>>>>>>> Sqlite3EntityTable of 'product' initialized successfuly
    flutter >>>>>>>>>>>>>>>>>>>>>>>>>>>> Sqlite3EntityTable of 'todos' initialized successfuly
    ...
    flutter: EXAMPLE 10 Sqlite3Entity Sequence SAMPLES-----------
    flutter: Sample Code:
