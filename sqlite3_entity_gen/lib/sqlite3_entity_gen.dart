...
part 'sqlite3_entity_base.dart';
part 'sqlite3_entity_form_gen.dart';
...
class Sqlite3EntityFormCreator {
  Sqlite3EntityFormCreator(this.tableModel, this.instancename);
  ...
  List<Sqlite3EntityTableBase> toTables() {
    final table = toSqlite3EntityTable(tableModel, instancename);
...
class Sqlite3EntityModelBuilder extends Sqlite3EntityModelBase {
  Sqlite3EntityModelBuilder(this.model, this.instancename);
  ...
  /// convert dartObject model const to Sqlite3EntityModelBase
  Sqlite3EntityModelBase toModel() {
  ...
  /// convert dartObject to Sqlite3EntityTableBase
  List<Sqlite3EntityTableBase>? toTableList(
      List<DartObject> objTables, String dbModelName,
      {List<Sqlite3EntityFieldType>? defaultColumns}) {
    final retVal = <Sqlite3EntityTableBase>[];
    //if (objTables != null) {
    print('sqlite3_entity_gen.DART: recognizing Tables ($dbModelName)');

    for (var obj in objTables) {
      //  print(   '-------------------------------------------------------ModelBuilder: ${getStringValue(obj, 'tableName')}');
      final table = toSqlite3EntityTable(
        obj,
        dbModelName,
  ...
  /// convert List<dartObject> to List<Sqlite3EntitySequenceBase>
  List<Sqlite3EntitySequenceBase>? toSequenceList(List<DartObject>? objSequences) {
    if (objSequences == null) {
      //  print('sqlite3_entity_gen.DART: toSequenceList() returned null');
      return null;
    }
    print('sqlite3_entity_gen.DART: recognizing Sequences ($modelName)');
    final sqfEntitySQEList = <Sqlite3EntitySequenceBase>[];
    for (var obj in objSequences) {
      try {
        sqfEntitySQEList.add(toSequence(obj));
      } catch (e) {
        print(
            'sqlite3_entity_gen.DART: recognizing Sequences ERROR. ($modelName) tried obj=>${obj.toString()}\nError Message:${e.toString()}');
      }
    }
...
/// convert dartObject to Sqlite3EntitySequenceBase
Sqlite3EntitySequenceBase toSequence(DartObject obj) {
  return Sqlite3EntitySequenceBase()
...
/// convert dartObject to Sqlite3EntityTableBase
Sqlite3EntityTableBase? toSqlite3EntityTable(DartObject obj, String dbModelName,
    {List<Sqlite3EntityFieldType>? defaultColumns}) {
  final String? _tableName = getStringValue(obj, 'tableName');

  if (_tableName == null) {
    print('__________toSqlite3EntityTable() returned null');
    return null;
  }
  final table = Sqlite3EntityTables.tableList
  ...
  final newTable = Sqlite3EntityTableBase()
  ...
  if (defaultColumns != null && defaultColumns.isNotEmpty) {
    ...
    for (Sqlite3EntityFieldType defaultField in defaultColumns) {
      final Sqlite3EntityFieldBase newField = Sqlite3EntityFieldBase(
  ...
  newTable.init();
  Sqlite3EntityTables.add(newTable);

  print(
      'Sqlite3EntityModelBuilder: $dbModelName/$_tableName added to Sqlite3EntityTables');
  return newTable;
}

/// get field properties from dartObject
Sqlite3EntityFieldType getFieldProperties(
    Sqlite3EntityFieldType field, DartObject obj) {
  final _retVal = field
    ...
    ..sequencedBy =
        obj.getField('sequencedBy').toString().contains('Sqlite3EntitySequence')
            ? toSequence(obj.getField('sequencedBy')!)
            : null;
  return _retVal;
}

/// convert dartObject to Sqlite3EntityFieldType
Sqlite3EntityFieldType toField(DartObject obj, String dbModelName,
    {List<Sqlite3EntityFieldType>? defaultColumns}) {
  final fieldName =
  ...
  if (obj.toString().startsWith('Sqlite3EntityFieldVirtual')) {
    return Sqlite3EntityFieldVirtualBase(fieldName!, dbType);
  } else if (obj.toString().startsWith('Sqlite3EntityFieldRelationship')) {
    final parentTable = toSqlite3EntityTable(
        obj.getField('parentTable')!, dbModelName,
        defaultColumns: defaultColumns);

    return Sqlite3EntityFieldRelationshipBase(
    ...
    final Sqlite3EntityFieldType retVal = Sqlite3EntityFieldBase(fieldName!, dbType);
    return getFieldProperties(retVal, obj);
  }
}

/// convert dartObject to List<Sqlite3EntityFieldType>
List<Sqlite3EntityFieldType>? toFields(
    List<DartObject>? objFields,
    String dbModelName,
    String tableName //bool keepFieldNamesAsOriginal
    ,
    {List<Sqlite3EntityFieldType>? defaultColumns}) {
  final sqfEntityFieldList = <Sqlite3EntityFieldType>[];
  // print('--------RECOGNIZING FIELDS:');
  if (objFields == null) {
    return <Sqlite3EntityFieldType>[];
  }
...
class Sqlite3EntityTables {
  static List<Sqlite3EntityTableBase>? tables;
  static List<Sqlite3EntityTableBase> get tableList =>
      tables ?? <Sqlite3EntityTableBase>[];

  static void add(Sqlite3EntityTableBase table) {
    tables = tables ?? <Sqlite3EntityTableBase>[]
      ..add(table);
  }
}
...
class Sqlite3EntityConverter {
  Sqlite3EntityConverter(this._m);
  final Sqlite3EntityModelBase _m;
  String _tableList() {
...
//  These classes was generated by Sqlite3Entity 
//  Copyright (c) 2019, All rights reserved. Use of this source code is governed by a
//  Apache license that can be found in the LICENSE file.

//  To use these Sqlite3Entity classes do following: 
...
class ${_m.modelName} extends Sqlite3EntityModelProvider {
  ${_m.modelName}() {
...
// BEGIN DATABASE MODEL
@Sqlite3EntityBuilder(${tocamelCase(_m.modelName)})
const ${tocamelCase(_m.modelName)} = Sqlite3EntityModel(
...
''';
  ...
  String _createModelTables() {
    ...
    final List<Sqlite3EntityTableBase> removeTables = <Sqlite3EntityTableBase>[];
    int index = 0;
    for (final table in _m.databaseTables!) {
...
class Table${table.modelName} extends Sqlite3EntityTableBase {
  ...
  static Sqlite3EntityTableBase? _instance;
  static Sqlite3EntityTableBase get getInstance {
...
const table${toCamelCase(table.tableName)} = Sqlite3EntityTable(
...
class Sequence${seq.modelName} extends Sqlite3EntitySequenceBase {
  ...
  String _createModelFields(Sqlite3EntityTableBase table) {
    ...
    for (final field in table.fields!) {
      ...
      if (field is Sqlite3EntityFieldVirtualBase) {
        strFields.writeln(
            'Sqlite3EntityFieldVirtualBase(\'${field.fieldName}\', ${field.dbType.toString()}),');
      } else if (field is Sqlite3EntityFieldRelationshipBase) {
        strFields.writeln(
            'Sqlite3EntityFieldRelationshipBase(${field.table == null || field.table!.tableName == table.tableName ? 'null' : 'Table${field.table!.modelName}.getInstance'}, ${field.deleteRule.toString()}${_getNullableValueField(field.relationType, 'relationType')}${_getNullableValueField(field.fieldName!, 'fieldName')}$commonProperties),');
      } else {
        strFields.writeln(
            'Sqlite3EntityFieldBase(\'${field.fieldName}\', ${field.dbType.toString()}$commonProperties),');
      }
    }
  ...
  String _createModelFieldsConst(Sqlite3EntityTableBase table) {
    final strFields = StringBuffer();

    for (final field in table.fields!) {
      if (field is Sqlite3EntityFieldRelationshipBase) {
        strFields.writeln(
            'Sqlite3EntityFieldRelationship(parentTable: ${field.table == null || field.table!.tableName == table.tableName ? 'null' : 'table${field.table!.modelName}'}, deleteRule: ${field.deleteRule.toString()}${_getNullableValueField(field.fieldName!, 'fieldName')}${field.isPrimaryKeyField != null && field.isPrimaryKeyField! ? _getNullableValueField(field.isPrimaryKeyField, 'isPrimaryKeyField') : ''}${_getNullableValueField(field.relationType, 'relationType')}${_getNullableValueField(field.manyToManyTableName, 'manyToManyTableName')}),');
      } else {
        strFields.writeln(
            'Sqlite3EntityField(\'${field.fieldName}\', ${field.dbType.toString()}),');
      }
    }
    return strFields.toString();
  }
  ...
  String createEntites() {
    ...
    if (_m.databaseTables != null) {
      ...
      for (var table in _m.databaseTables!) {
        ...
        modelString
          ...
          ..writeln(Sqlite3EntityObjectBuilder(table).toString())
          // ..printToDebug('1: ${table.tableName}')
          ..writeln(Sqlite3EntityObjectField(table).toString())
          //  ..printToDebug('2: ${table.tableName}')
          ..writeln(Sqlite3EntityObjectFilterBuilder(table,
                  _m.formTables != null && _m.formTables!.contains(table))
              .toString())
          //  ..printToDebug('3: ${table.tableName}')
          ..writeln(Sqlite3EntityFieldBuilder(table).toString())
          //  ..printToDebug('4: ${table.tableName}')
          ..writeln(Sqlite3EntityObjectManagerBuilder(table).toString());
      }
    }
    ...
    if (_m.sequences != null) {
      for (var sequence in _m.sequences!) {
        modelString.writeln(
            Sqlite3EntitySequenceBuilder(sequence, _m.modelName!).toString());
      }
    }
    modelString
      ..writeln(
          '''class ${_m.modelName}SequenceManager extends Sqlite3EntityProvider {
  ...
  String createControllers() {
    ...
    for (var table in _m.formTables!) {
      modelString
        ...
        ..writeln(Sqlite3EntityControllerBuilder(table, _m.formTables!)
            .toControllersCode()
            .toString());
    }
...
class _DbModel extends Sqlite3EntityModelBase {}
